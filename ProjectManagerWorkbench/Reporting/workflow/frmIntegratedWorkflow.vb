...
Public Class frmIntegratedWorkflow
    ...
    Private Sub createRevenueData()
        ...
        Try
            ...
            'With this information go into the backlog and grab the relevant information
            Dim dsBak As DataSet = db.SecureQueryParams("SELECT ISNULL(SUM(extended_price),0) extendedPrice, datepart(month, promise_date) month_promised, datepart(month,getdate()) currentMonth  FROM salesOrderLinesView " & _
                                                        "   WHERE organization_id = @1 " & _
                                                        " AND datepart(year,promise_date) = datepart(year,getDate()) AND substring(CONVERT(varchar,CONVERT(integer,order_no)),2,1) <> '5' " & ds.Tables(0).Rows(0).Item(0) & _
                                                        " group by datepart(month,promise_date)     " & _
            ...
            'With this information go into the backlog and grab the relevant information
            dsBak = db.SecureQueryParams("SELECT ISNULL(SUM(extended_price),0) extendedPrice, datepart(month, promise_date) month_promised, datepart(month,getdate()) currentMonth  FROM salesOrderLinesView " & _
                                                        "   WHERE organization_id = @1 " & _
                                                        " AND datepart(year,promise_date) = datepart(year,getDate()) AND substring(CONVERT(varchar,CONVERT(integer,order_no)),2,1) = '5' " & ds.Tables(0).Rows(0).Item(0) & _
                                                        " group by datepart(month,promise_date)     " & _
                                                        " ORDER BY datepart(month,promise_date)     ", 255)
    ...
    Private Sub addAnotherParameter(ByVal filterName As String, ByVal seriesName As String, ByVal seriesTotalName As String, ByVal buyUnits As String)
        ...
        'With this information go into the backlog and grab the relevant information
        Dim dsBak = db.SecureQueryParams("SELECT ISNULL(SUM(extended_price),0) extendedPrice, datepart(month, promise_date) month_promised, datepart(month,getdate()) currentMonth  FROM salesOrderLinesView " & _
                                                    "   WHERE organization_id = @1 " & _
                                                    " AND datepart(year,promise_date) = datepart(year,getDate()) AND substring(CONVERT(varchar,CONVERT(integer,order_no)),2,1)  " & buyUnits & ds.Tables(0).Rows(0).Item(0) & _
                                                    " group by datepart(month,promise_date)     " & _
                                                    " ORDER BY datepart(month,promise_date)     ", 255)
